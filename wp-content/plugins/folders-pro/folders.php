...
if(!defined("WCP_PRO_FOLDERS_PLUGIN_FILE")) {
    define('WCP_PRO_FOLDERS_PLUGIN_FILE', __FILE__);
}
if(!defined("WCP_PRO_FOLDERS_PLUGIN_BASE")) {
    define('WCP_PRO_FOLDERS_PLUGIN_BASE', plugin_basename(WCP_PRO_FOLDERS_PLUGIN_FILE));
}
if(!defined("WCP_DS")) {
    define("WCP_DS", DIRECTORY_SEPARATOR);
}
if(!defined("WCP_PRO_FOLDER_URL")) {
    define('WCP_PRO_FOLDER_URL', plugin_dir_url(__FILE__));
}

if(!defined("WCP_PRO_FOLDER_DIR")) {
    define('WCP_PRO_FOLDER_DIR', plugin_dir_path(__FILE__));
}

/*PRO variable*/
define("WCP_PRO_FOLDER_API_URL","https://go.premio.io/");
define("WCP_PRO_FOLDER_PR0DUCT_ID","687");
define("WCP_PRO_FOLDER_VERSION","3.0.4");

if(!function_exists("folders_pro_clear_all_caches")) {
    function folders_pro_clear_all_caches()
    {
...
include_once plugin_dir_path(__FILE__) . "includes/plugins.folders.php";
//include_once plugin_dir_path(__FILE__)."includes/media.clean.php";

register_activation_hook( __FILE__, array( 'WCP_Pro_Folders', 'activate' ) );
register_deactivation_hook( __FILE__, array( 'WCP_Pro_Folders', 'deactivate' ) );
WCP_Pro_Folders::get_instance();


/* Affiliate Class*/
if(is_admin()) {
    include_once plugin_dir_path(__FILE__) . "includes/class-review-box.php";
    include_once plugin_dir_path(__FILE__) . "includes/license-key-box.php";
}

/*To get updates*/
include_once plugin_dir_path(__FILE__)."includes/plugin.updates.php";
$licenseKey = get_option("wcp_folder_license_key");
new Folder_Plugin_Updater( WCP_PRO_FOLDER_API_URL, __FILE__, array(
        'version' 	=> WCP_PRO_FOLDER_VERSION,
        'license' 	=> $licenseKey,
...
if(!function_exists('premio_folders_pro_plugin_check_for_setting')) {
    function premio_folders_pro_plugin_check_for_setting() {
    ...
    add_action( 'plugins_loaded', 'premio_folders_pro_plugin_check_for_setting' );
}
