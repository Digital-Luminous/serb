...
class Folders_Import_Export
{
    ...
    public function folders_export()
    {
        ...
        if (!isset($postData['nonce']) || empty($postData['nonce'])) {
            ...
            $nonce = WCP_Pro_Folders::sanitize_options($postData['nonce']);
        ...
        if ($errorCounter == 0) {
            ...
            foreach($post_types as $post =>$value) {
                $folder_type = WCP_Pro_Folders::get_custom_post_type($post);
    ...
    public function folders_import()
    {
        ...
        $success = 0;
        if ($errorCounter == 0) {
            $fileContent = WCP_Pro_Folders::sanitize_options($postData['uploaded_data']);
            $fileContent = json_decode($fileContent,true);
            foreach($fileContent as $content) {
                $post_type = $content['post_type'];
                $folders = $content['folders'];
                $folder_type = WCP_Pro_Folders::get_custom_post_type($post_type);
    ...
    public function insert_imported_folders($folders,$folder_type) {
        ...
        foreach($folders as $folder) {
            ...
            if(empty($term_id)) {
                $slug = WCP_Pro_Folders::create_slug_from_string($folder_name) . "-" . time();
