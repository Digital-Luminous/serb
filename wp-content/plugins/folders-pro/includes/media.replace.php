...
class foldersPro_replace_media {
...
//
        ...
        if(isset($customize_folders['enable_media_trash']) && $customize_folders['enable_media_trash'] == "off" && isset($customize_folders['enable_media_trash_changed']) && $customize_folders['enable_media_trash_changed'] == 1) {
            add_action('admin_notices', array($this, 'enable_media_notices_trash_changed'));
        }
    ...
    public function enable_media_notices_trash_changed() {
        $is_defined = defined( 'MEDIA_TRASH' );
        if($is_defined && MEDIA_TRASH) {
            if (isset($_REQUEST['page']) && ($_REQUEST['page'] == "wcp_folders_settings" || $_REQUEST['page'] == "folders-media-cleaning")) { ?>
                <style>
                ...
                <div class="notice notice-info premio-notice media-trash-notice">
                    <div class="media-trash-notice-message">
                        <p><b><?php esc_html_e("Disable Trash functionality for Media", "folders") ?></b></p>
                        To implement this change, You need to remove <code>define( 'MEDIA_TRASH', true );</code> from the <a href="https://wordpress.org/support/article/editing-wp-config-php/" rel="noopener noreferrer" target="_blank">wp-config.php</a> file.
                    </div>
                    <div class="media-trash-notice-footer">
                        <a class="button button-primary" href="#"><?php esc_html_e("Automatically remove this line", "folders") ?><span class="spinner trash-spinner"></span></a>
                    </div>
                </div>
                ...
                <script>
                    (function($) {
                        "use strict";
                        $(document).ready(function(){
                            $(document).on("click", ".media-trash-notice-footer a:not(.disabled), .remove-in-config-file:not(.disabled)", function(e){
                                ...
                                $.ajax({
                                    ...
                                    data: {
                                        action: "premio_folder_update_wp_config_remove",
                                        nonce: "<?php echo esc_attr(wp_create_nonce("remove_media_status_in_wp_config")) ?>"
                                    },
                                    type: 'post',
                                    dataType: "json",
                                    success: function(response) {
                                        if(response.status == 1) {
                                            ...
                                            $("#wp-config-update-notice-remove").show();
                                            $("#folder-trash-message").remove();
                                        }
                                    }
                                });
            ...
            if (isset($_REQUEST['page']) && $_REQUEST['page'] == "folders-media-cleaning") { ?>
                ...
                <div class="notice notice-info premio-notice media-trash-notice">
                    <div class="media-trash-notice-message">
                        ...
                        <a class="button button-primary float-right" href="<?php echo esc_url(admin_url("upload.php?page=folders-media-cleaning&enable_trash=".esc_attr(wp_create_nonce("folders_enable_media_trash")))) ?>" >Enable</a>
                    </div>
                </div>
            <?php }
        }
    ...
    public function admin_notices() {
        ...
        if ( !($is_defined && MEDIA_TRASH )) {
            ...
            if ((isset($current_screen->base) && $current_screen->base == "upload") || (isset($_REQUEST['page']) && ($_REQUEST['page'] == "wcp_folders_settings" || $_REQUEST['page'] == "folders-media-cleaning"))) { ?>
                ...
                <?php if(isset($_REQUEST['mode']) && $_REQUEST['mode'] == "list" && isset($_REQUEST['attachment-filter']) && $_REQUEST['attachment-filter'] == "trash") {
                    $isPopUpShown = get_option("is_folders_trash_media_shown");
                    if($isPopUpShown != 1) {
                        add_option("is_folders_trash_media_shown", 1);
                        ?>
                            ...
                            <div class="popup-form-content">
                                <div class="popup-form-data">
                                    ...
                                    <form action="" method="post" id="save-folder-form">
                                        <div class="add-update-folder-title"><?php esc_html_e("Rewrite wp-config.php to enable Trash", "folders") ?></div>
                                        <div class="folder-note">
                                            To enable Trash functionality in Media, Add the following one line <code>define( 'MEDIA_TRASH', true );</code> in <a href="https://wordpress.org/support/article/editing-wp-config-php/" rel="noopener noreferrer" target="_blank">wp-config.php</a> file just before the line that says "That’s all, stop editing!"
                                        </div>
                                        <div class="folder-form-buttons">
                                            <a href="javascript:;" class="form-cancel-btn"><?php esc_html_e("I'll do it manually", "folders") ?></a>
                                            <button type="submit" class="form-submit-btn write-in-config-file"><?php esc_html_e("Automatically write this line", "folders") ?></button>
                                        </div>
                                    </form>
                                </div>
                            </div>
    ...
    public function update_wp_config_remove() {
        ...
        if($errorCounter == 0) {
            ...
            try {
                ...
                $customize_folders = get_option('customize_folders');
                if($isExists) {
                    $customize_folders['enable_media_trash_changed'] = 0;
                    update_option('customize_folders',$customize_folders);
                }
    ...
    public function replace_media_file_script() {
        wp_enqueue_script('folders-media-replace-js', WCP_PRO_FOLDER_URL . 'assets/js/replace-file-name.js', array('jquery'), WCP_PRO_FOLDER_VERSION, true);
    ...
    public function folders_admin_css_and_js($page) {
        if($page == "media_page_folders-replace-media" || $page == "admin_page_folders-replace-media") {
            wp_enqueue_style('folders-replace-media', plugin_dir_url(dirname(__FILE__)) . 'assets/css/replace-media.css', array(), WCP_PRO_FOLDER_VERSION);
            wp_enqueue_style('folders-datetimepicker', plugin_dir_url(dirname(__FILE__)) . 'assets/css/datetimepicker.css', array(), WCP_PRO_FOLDER_VERSION);

            wp_enqueue_script('folders-datetimepicker', plugin_dir_url(dirname(__FILE__)) . 'assets/js/datetimepicker.js', array(), WCP_PRO_FOLDER_VERSION, true);
            wp_enqueue_script('folders-simpledropit', plugin_dir_url(dirname(__FILE__)) . 'assets/js/simpledropit.min.js', array(), WCP_PRO_FOLDER_VERSION, true);
            wp_enqueue_script('folders-replace-media', plugin_dir_url(dirname(__FILE__)) . 'assets/js/replace-media.min.js', array(), WCP_PRO_FOLDER_VERSION, true);
...
//            wp_enqueue_script('jquery-ui-datepicker');
    ...
    public function replace_meta_box($post) {
        if(current_user_can("upload_files")) {
            $link = $this->getMediaReplaceURL($post->ID);
            echo "<p><a style='background: ".esc_attr($this->button_color)."; border-color: ".esc_attr($this->button_color)."; color:#ffffff' href='" . esc_url($link) . "' class='button-secondary'>" . esc_html__("Upload a new file", "folders") . "</a></p><p>" . esc_html__("Click on the button to replace the file with another file", "folders") . "</p>";
        }
    }
    ...
    public function change_file_name_box($post) {
        if(current_user_can("upload_files")) {?>
        <p>
            <input type="hidden" name="premio_change_nonce" value="<?php echo esc_attr(wp_create_nonce("premio_change_file_name_".$post->ID)) ?>">
            <input type="hidden" name="premio_change_file_name" value="no">
            <label for="change_file_name"><input <?php checked($this->replace_media_title, "on") ?> type="checkbox" id="change_file_name" name="premio_change_file_name" value="yes"> <?php esc_html_e("Change file name according to title", "folders") ?></label>
        </p>
        <?php
        }
    }
    ...
    public function attachment_editor($form_fields, $post)
    {
        ...
        if(current_user_can("upload_files")) {
            ...
            $form_fields["folders"] = array(
                ...
                "input" => "html",
                "html" => "<a style='background: ".esc_attr($this->button_color)."; border-color: ".esc_attr($this->button_color)."; color:#ffffff' href='" . $link . "' class='button-secondary'>" . esc_html__("Upload a new file", "folders") . "</a>", "helps" => esc_html__("Click on the button to replace the file with another file", "folders")
            );
        }
    ...
    public function attachment_replace_name_with_title($form_fields, $post)
    {
        ...
        if(current_user_can("upload_files")) {
            $form_fields["replace_file_name"] = array(
                "label" => esc_html__("Replace media", "folders"),
                "input" => "html",
                "html" => "<label for='attachment_title_".esc_attr($post->ID)."' data-post='".esc_attr($post->ID)."' data-nonce='" . esc_attr(wp_create_nonce('change_attachment_title_' . $post->ID)) . "'><input id='attachment_title_".esc_attr($post->ID)."' type='checkbox' class='folder-replace-checkbox' value='".esc_attr($post->ID)."'>" . esc_html__("Update file name with title") . "</label><button style='background: ".esc_attr($this->button_color)."; border-color: ".esc_attr($this->button_color)."; color:#ffffff' type='button' class='button update-name-with-title' >" . esc_html__("Update", "folders") . " <span class='spinner'></span></button>",
                "helps" => ""
            );
        }
            ...
            if($_FILES['new_media_file']['error'] == 0) {
                ...
                if($wp_attached_file !== false) {
                    ...
                    if($replacement_option == "replace_file_with_name" && isset($_REQUEST['new_folder_option']) && $_REQUEST['new_folder_option'] && isset($_REQUEST['new_folder_path']) && !empty($_REQUEST['new_folder_path'])) {
                        $this->validate_path($_REQUEST['new_folder_path']);
                    ...
                    if ($status) {
                        ...
                        if(isset($_REQUEST['date_options']) && !empty($_REQUEST['date_options'])) {
                            if($_REQUEST['date_options'] == "replace_date") {
                                ...
                                $custom_date = gmdate("Y-m-d H:i:s", strtotime($custom_date));
                                if($custom_date !== false && $this->validate_date($custom_date, "Y-m-d H:i:s")) {
                                    $datetime  =  gmdate("Y-m-d H:i:s", strtotime($custom_date));
...
$foldersPro_replace_media = new foldersPro_replace_media();
