...
jQuery(document).on("change", "#media-attachment-dynamic-filter", function(){
    if(jQuery(this).val() != 'dynamic-folders') {
        jQuery("#media-attachment-taxonomy-filter").val("all").trigger("change");
    }
});
...
jQuery(window).load(function(){
    if(jQuery("#dynamic-tree-folders .jstree-anchor.jstree-clicked").length) {
        jQuery("#dynamic-tree-folders .jstree-anchor.jstree-clicked").trigger("click");
    }
});
var selectedFolderMediaId = -1;
        ...
        if(!jQuery("#wcp-content").length) {
            if(folders_media_options.is_key_active == 0 && folders_media_options.folders >= 10) {
                ...
                if (!jQuery("#custom-folder-media-popup-form").length) {
                    ...
                    jQuery(".media-popup-form").append('<div class="folder-form-buttons"><a href="javascript:;" class="remove-media-form">Cancel</a><a href="'+folders_media_options.activate_url+'" target="_blank" class="form-submit-btn" style="width: 120px">Activate License Key</button></div>');
                }
            } else {
                if (jQuery("#custom-folder-media-popup-form").length) {
                    jQuery("#custom-folder-media-popup-form").remove();
