        ...
        <div class="replace-media-page">
            ...
            <div class="replace-name-settings">
                ...
                <div class="replace-name-settings-right">
                    <div class="inline-switch">
                        ...
                        <input type="checkbox" name="replacement_option" id="replacement_option" value="replace_file_with_name" class="sr-only">
            ...
            <div class="date-options">
                <div class="inline-radio">
                    <input class="sr-only" type="radio" name="date_options" value="replace_date" id="replace_date">
                ...
                <div class="inline-radio">
                    <input class="sr-only" type="radio" name="date_options" value="custom_date" id="select_custom_date">
                ...
                <div class="custom-date" id="custom-path">
                    ...
                    <div class="inline-checkbox">
                        <input type="checkbox" class="sr-only" id="new_folder_option" name="new_folder_option" value="1">
                    ...
                    <span class="inline">
                        ...
                        if(isset($upload_dir['basedir'])) {
                            $scan = scandir($upload_dir['basedir']);
                            foreach($scan as $file) {
                                if (is_dir($upload_dir['basedir']."/".$file)) {
                                    if(is_numeric($file) && $file < $endDir) {
                                        $endDir = $file;
                                    }
                                }
                            }
                        ...
                        ?>
                        <select name="new_folder_path" class="media-path-select">
                            <?php
                            $year = gmdate("Y");
                            for($i=intval(gmdate("m"));$i>=1;$i--) {
                                $date = $year."-".$i."-01";
                                if(is_dir($upload_dir['basedir'].DIRECTORY_SEPARATOR.gmdate("Y", strtotime($date)).DIRECTORY_SEPARATOR.gmdate("m", strtotime($date)))) {
                                    $optionVal = gmdate("Y/m", strtotime($date)); ?>
                                    <option value="<?php echo esc_attr($optionVal) ?>"><?php echo esc_attr($optionVal) ?></option>
                                <?php }
                            }
                            if($endDir != $year) {
                                for($currentYear= ($year - 1); $currentYear>=$endDir; $currentYear--) {
                                    for($i=12;$i>=1;$i--) {
                                        $optionVal = gmdate("Y/m", strtotime($currentYear."-".$i."-01"));
                                        if(is_dir($upload_dir['basedir'].DIRECTORY_SEPARATOR.gmdate("Y", strtotime($optionVal)).DIRECTORY_SEPARATOR.gmdate("m", strtotime($optionVal)))) { ?>
                                            <option value="<?php echo esc_attr($optionVal) ?>"><?php echo esc_attr($optionVal) ?></option>
                                            <?php
                                        }
                                    }
